...
public class QuorumPeerConfig {
    ...
    //autopurge.snapRetainCount与autopurge.purgeInterval配套使用。
    // autopurge.purgeInterval。3.4.0及之后版本，ZK提供了自动清理事务日志和快照文件的功能，这个参数指定了清理频率，单位是小时，需要配置一个1或更大的整数，
    // 默认是0，表示不开启自动清理功能。
    //autopurge.snapRetainCount。这个参数指定了需要保留的文件数目。默认是保留3个。且最低就是3个。
    ...
    public void parse(String path) throws ConfigException {
        ...
        try {
            ...
            Properties cfg = new Properties();//属性初始化
            FileInputStream in = new FileInputStream(configFile);
            try {
                //把zoo.cfg这种配置文件读出来。放到cfg中。
                cfg.load(in);
            } finally {
                in.close();
            }
     ...
     * @throws ConfigException
     */
    public void parseProperties(Properties zkProp)//Properties是继承了hashtable，一般不是都不会用了吗？
    throws IOException, ConfigException {
        ...
        //集群使用。quorumEnableSasl就是开启集群认证。quorumServerRequireSasl就是各个server间需要认证。
        //这个判断的意思就是如果没有开启sasl认证，并且集群间又需要认证，肯定是不合理的，所以直接抛出异常。至于认证的具体作用我也不是很清楚。
        ...
        if (!quorumEnableSasl && quorumLearnerRequireSasl) {//道理跟前面一样。
            throw new IllegalArgumentException(
                    QuorumAuth.QUORUM_SASL_AUTH_ENABLED
                    + " is disabled, so cannot enable "
        ...
        //这里就是判断是否为集群。
        // 如果配置文件中有以server.开头的就是集群的意思。否则就是单机
        if (servers.size() == 0) {//单机
            if (observers.size() > 0) {
            ...
            // mode.
            return;
        } else if (servers.size() == 1) {//server必须大于1
            if (observers.size() > 0) {
